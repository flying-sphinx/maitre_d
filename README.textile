...
As shown above, you can use Maître d' Rack API for both Heroku and (with a custom configuration argument) Cloud Control. Mount them to wherever you see fit. Once upon a time Heroku expected the endpoints to be at /heroku, but now things are a little more flexible.
