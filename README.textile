...
mount MaitreD::Heroku::API.new(MaitreD::CloudControl) => '/cloudcontrol'</code></pre>

h3. Without Rails

As shown above, you can use Maître d' Heroku Rack API for both Heroku and (with a custom configuration argument) Cloud Control. Mount them to wherever you see fit. Once upon a time Heroku expected the endpoints to be at /heroku, but now things are a little more flexible.
